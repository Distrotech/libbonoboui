...
/* Initializer */
void hello_model_init (Hello *obj, BonoboEmbeddable *bonobo_object)
{
...
void hello_model_clear (Hello *obj)
{
    if (obj->text)
	g_free (obj->text);
    obj->text = NULL;
...
hello_model_update_view (BonoboView *view, gpointer user_data)
{
    ...
    view_data = (HelloView*)gtk_object_get_data ( GTK_OBJECT (view), "view_data");;
...
/* Change object data */
void
hello_model_set_text (Hello *obj,
		       gchar *text)
{
    hello_model_clear (obj);
