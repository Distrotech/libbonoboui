...
static void bonobo_socket_set_focus_child          (GtkContainer *container,
						    GtkWidget *widget);
...
bonobo_socket_class_init (BonoboSocketClass *class)
{
	...
	container_class->set_focus_child = bonobo_socket_set_focus_child;
}
...
/* Gives the input focus to the child window.  We are essentially acting like a
 * little window manager.
 */
static void
focus_child_window (BonoboSocket *socket)
{
	BonoboSocketPrivate *priv;
...
/* Gives the input focus to our toplevel window, i.e. takes it away from the
 * child.
 */
static void
focus_toplevel_window (BonoboSocket *socket)
{
	BonoboSocketPrivate *priv;
	GtkWidget *toplevel;
	XWindowAttributes attr;
	...
	toplevel = gtk_widget_get_ancestor (GTK_WIDGET (socket), GTK_TYPE_WINDOW);
...
#if 0
	fprintf (stderr, "socket_focus_in: Socket focus in event\n");
#endif
	if (priv->gave_focus)
		focus_child_window (socket);
...
bonobo_socket_focus_out_event (GtkWidget *widget, GdkEventFocus *event)
{
	...
	/* This is a synthetic event sent to us by the GTK+ focusing system, so
	 * we can claim the focus for our toplevel.  Non-synthetic events are
...
bonobo_socket_focus (GtkContainer *container, GtkDirectionType direction)
{
	...
	retval = bonobo_control_frame_focus (priv->frame, direction);
...
/* Set_focus_child handler for the socket widget */
static void
bonobo_socket_set_focus_child (GtkContainer *container, GtkWidget *widget)
{
#if 0
	if (!widget)
		fprintf (stderr, "socket_set_focus_child(): NULL\n");
	else
		fprintf (stderr, "socket_set_focus_child(): some other widget\n");
#endif
	if (parent_class->set_focus_child)
		(* parent_class->set_focus_child) (container, widget);
}
...
bonobo_socket_filter_func (GdkXEvent *gdk_xevent, GdkEvent *event, gpointer data)
{
	...
	case FocusIn:
#if 0
		fprintf (stderr, "socket_filter: FocusIn event from the child!\n");
#endif
		if (xevent->xfocus.mode == EMBEDDED_APP_WANTS_FOCUS) {
#if 0
			fprintf (stderr, "\tand it wants focus!\n");
#endif
			if (!priv->gave_focus) {
#if 0
				fprintf (stderr, "\thad not given focus to the child; giving it\n");
#endif
				gtk_widget_grab_focus (GTK_WIDGET (socket));
				focus_child_window (socket);
			} else {
#if 0
				fprintf (stderr, "\thad given focus to child; not giving focus\n");
#endif
			}
		}
	...
	case FocusOut:
		...
		else if (xevent->xfocus.window == GDK_WINDOW_XWINDOW (priv->plug_window))
